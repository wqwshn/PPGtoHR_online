...
def _crop_to_analysis_window(HR: np.ndarray, pre_motion_seconds: float = 30.0) -> np.ndarray:
    """Keep only 30 s before first motion through the last motion window."""
    if HR.size == 0:
        return HR
    motion_times = HR[HR[:, 7] == 1, 0]
    if motion_times.size == 0:
        return HR
    start = max(float(HR[0, 0]), float(motion_times[0]) - pre_motion_seconds)
    end = float(motion_times[-1])
...
def solve_from_arrays(
    ...
    HR = _crop_to_analysis_window(np.asarray(rows, dtype=float))
